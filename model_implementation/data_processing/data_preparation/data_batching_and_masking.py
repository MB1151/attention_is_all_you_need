...
class Batch:
    ...
    def __init__(self, src_batch: Tensor, tgt_batch: Tensor, pad_token_id: int, device: str):
        ...
        # It might be tempting to make the src_mask of shape [batch_size, 1, seq_len, seq_len] but, for the 
        # src_mask to be used with self attention in Encoder, the shape should be 
        # [batch_size, num_heads, seq_len, seq_len] and for the src_mask to be used with source attention in
        # Decoder, the shape should be [batch_size, num_heads, seq_len - 1, seq_len]. So, we will keep the 
        # shape of src_mask as [batch_size, 1, 1, seq_len] and then let the model handle the broadcasting of 
    ...
    def construct_target_mask(self, tgt: Tensor, pad_token_id: int) -> Tensor:
        """Creates the mask for the target sentences. There is really no value in using the padding mask in 
           the target sentences. The padding tokens are only at the end of the sentence and the future 
           tokens are not allowed to attend to the earlier tokens because of the look_ahead_mask already. 
           This means the padding tokens are already masked for the actual tokens (part of the sentence) 
        ...
        Args:
            tgt (Tensor): Tensor containing the tgt sentences in the batch. Contains token ids.
