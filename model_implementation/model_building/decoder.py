...
# Refer to 'step_13_decoder.ipynb' (add link to the notebook) notebook for a detailed explanation of each line of
# code in this file.
...
# Refer to 'step_5_data_batching_and_masking.ipynb' notebook to understand masking better.
#
# Here, lets try to understand how the shapes change when we calculate source_attention above.
# queries from tgt:    [batch_size, num_heads, tgt_seq_len, d_k]
# keys from encoder:   [batch_size, num_heads, src_seq_len, d_k]
# values from encoder: [batch_size, num_heads, src_seq_len, d_k]
#
# attention_scores = queries * keys^{transpose}  --> * here represents matrix multiplication.
# attention_scores: [batch_size, num_heads, tgt_seq_len, src_seq_len]
#
# attention_heads = attentions_scores * values  --> * here represents matrix multiplication.
# attention_heads: [batch_size, num_heads, tgt_seq_len, d_k]
# output of source attention calculation: [batch_size, num_heads, tgt_seq_len, d_k]
