...
def construct_attention_heads(queries: Tensor, 
    ...
    if mask is not None:
        ...
        # We used 'masked_fill_' instead of 'masked_fill' to make it an in-place operation. This was also responsible for some
        # of the Cuda memory leaks that I was facing.
        attention_scores.masked_fill_(mask == False, float('-1e9'))
    ...
    def forward(self, query_input: Tensor, key_input: Tensor, value_input: Tensor, mask: Optional[Tensor]=None) -> Tensor:
        ...
        Args:
            ...
            mask (Tensor): Mask to be applied to the attention scores. Default is None. Same mask will 
                           ...
                           mask: [batch_size, 1, 1, seq_len] if the mask is for the source sequences.
                           mask: [batch_size, 1, seq_len - 1, seq_len - 1] if the mask is for the target sequences. 
        ...
        # the 'seq_len' for the keys, values comes from Encoder output (i.e., src sentences) and the 'seq_len' for the queries comes
        # from decoder input (i.e., tgt sentences) in source attention. The sequence length for tgt sentences is (seq_len - 1) if 
        # 'seq_len' is considered as the number of tokens in each of the source sequences. So, we need to infer the size of the 
        # dimension from the original tensor instead of harcoding it from the query_input tensor. You can try it by hardcoding 
        # the seq_len (instead of setting it to -1) for keys and values and see the error you get.
